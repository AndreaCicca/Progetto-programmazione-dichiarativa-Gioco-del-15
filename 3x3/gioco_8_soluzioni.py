...
# Funzione per generare una configurazione valida del gioco dell'8
def generate_valid_configuration():
    tiles = list(range(9))  # Cifre da 0 a 8, dove 0 rappresenta lo spazio vuoto
    while True:
...
def save_configuration_to_file(tiles, filename="3x3.json"):
    try:
...
def valid_moves(zero_pos):
    ...
    if row > 0: moves.append(-3)  # Su
    if row < 2: moves.append(3)   # Giù
    if col > 0: moves.append(-1)  # Sinistra
    if col < 2: moves.append(1)   # Destra
    return moves
...
def manhattan_distance(tiles):
    ...
    for i, tile in enumerate(tiles):
        if tile != 0:
            correct_pos = 8 - tile  # Posizione corretta invertita
...
def solve_puzzle(start_tiles):
    ...
    while frontier:
        ...
        if current_tiles == list(range(8, -1, -1)):
            return path
