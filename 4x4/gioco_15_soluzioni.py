...
# Funzione per generare una configurazione valida del gioco del 15
def generate_valid_configuration():
    tiles = list(range(16))  # Cifre da 0 a 15, dove 0 rappresenta lo spazio vuoto
    while True:
...
def save_configuration_to_file(tiles, filename="4x4.json"):
    try:
...
def manhattan_distance(tiles):
    ...
    for i, tile in enumerate(tiles):
        if tile != 0:
            correct_pos = 15 - tile  # Posizione corretta invertita
...
def solve_puzzle(start_tiles):
    ...
    while frontier:
        ...
        if current_tiles == list(range(15, -1, -1)):
            return path
